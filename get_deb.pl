...
my $remote_packages 
      = "$base/dists/$release/$group/binary-armhf/Packages.gz";

# This is the local Packages file
my $packages = "$tmp/Packages-$group";

# This is the local compressed Packages file
my $packagescomp = "$tmp/Packages-$group.gz";
...
sub find_package_in_index ($)
  ...
  while (<PACKAGES>)
    ...
    if ($found)
      ...
      if ($line)
        {
	if ($line !~ 'Size:' && $line !~ 'MD5sum:' && $line !~ 'Filename:')
          { print LIST "$line\n"; }
